...
class GreatUXPathCompleter(PathCompleter):
  ...
  def __init__(
    ...
    Args:
        only_dorectories (bool): If True, only directories will be
                                 returned, but no files. Defaults to False.
        
        get_paths (Callable[[], List[str]], optional): Callable which
                  returns a list of directories to look into when the 
                  user enters a relative path. If None, set to (lambda: 
                  ["."]). Defaults to None.
        
        file_filter (Callable[[str], bool], optional): Callable which
                  takes a filename and returns whether this file should
                  show up in the completion. ``None`` when no filtering
                  has to be done. Defaults to None.
        
        min_input_len (int): Don't do autocompletion when the input string
                             is shorter. Defaults to 0.
        
        expanduser (bool): If True, tilde (~) is expanded. Defaults to False.
    
    Raises:
        ValueError: If any of the by `get_paths_ returned directories does not
                    exist.
    """
  ...
  def get_completions(
    ...
    for completion in completions:
      ...
      #source: https://pt.stackoverflow.com/questions/92921/para-que-serve-o-yield
      yield completion
...
def path(
  ...
  **kwargs: Any,
  ...
  Args:
      ...
      defaults: Default return value (single value)
      ...
      complete_style: How autocomplete menu would be shown, it could be ``COLUMN``
                      ...
                      :class:`promp_toolkit.shortcuts.CompleteStyle`.
      ...
      validate: Require the entered value to pass a validation. The 
                ...
                returning a boolean, or a class reference to a subclass
                of the prompt toolkit Validator class.
      ...
      file_filter: Optional Callable to filter suggested paths. Only paths
                   ...
                   the suggested paths. This does not valdate the typed paths, e.g.
  ...
  def next_segment(event: KeyPressEvent):
    currentBuffer = event.app.current_buffer

    if currentBuffer.complete_state:
      currentBuffer.complete_state = None
    
    current_path = currentBuffer.document.text
    if not current_path.endswith(os.path.sep):
      currentBuffer.insert_text(os.path.sep)
    
    currentBuffer.start_completion(select_first=False)
