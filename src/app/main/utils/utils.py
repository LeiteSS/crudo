...
def used_kwargs(kwargs: Dict[str, Any], func: Callable[..., Any]) -> Dict[str, Any]:
    ...
    Args:
        kwargs: All available kwargs. Kwargs mean Keywords Arguments (source: https://pythonacademy.com.br/blog/args-e-kwargs-do-python)
...
async def activate_prompt_toolkit_async_mode() -> None:
    ...
    Need to be async, so we use the right event loop in py 3.5
    """
    global ACTIVATED_ASYNC_MODE

    if not is_prompt_toolkit_v3():
        # Tell prompt_toolkit to use asynctio for the event loop.
